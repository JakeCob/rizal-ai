...
| 17 | Token exercises grade by exact sequence, so a second natural word order (Tagalog allows several) is marked wrong. Authors currently avoid ambiguous sentences; the reviewer flags any answer whose tiles admit another natural order. | Plan 004 | Add an `accepted_orders` list to the token exercise contracts and teach both graders; then re-author the avoided sentences. |
| 18 | Tile banks can hold two tiles that differ only by case (Galit and galit), which a learner working by ear can tap in the wrong order. Authors avoid repeated words in listen_tap transcripts for now. | Plan 004 | When item 17 lands, case-fold tile matching for listen_tap. |
| 19 | Unit.published is seeded but never read by the API or the web app. Today a unit with only locked lessons is the intended "locked horizon", so the flag has no meaning. | Plan 004 | Decide whether an unpublished unit hides its lessons entirely, then enforce it in the published-lesson helper and the tree query, or drop the field from the contract. |
...
| 22 | A lesson the learner completed and that is later unpublished still shows as a tappable done node on the tree, and opening it returns 404. | Plan 004 | Decide whether the tree hides or disables done-but-unpublished lessons when the first lesson is ever unpublished. |
