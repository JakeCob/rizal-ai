...
from rizalai.db.models import Exercise, ExerciseAttempt, ReviewQueue, UserProgress
...
async def _seed_with_hidden_lesson(db, tmp_path) -> None:
    """Fixture content plus an unpublished copy of the scaffold lesson, so an
    unpublished lesson has real exercises in the database."""
    ...
    lesson["slug"] = "hidden-lesson"
    lesson["published"] = False
    (unit_dir / "05-hidden.yaml").write_text(yaml.safe_dump(lesson, allow_unicode=True), encoding="utf-8")
    unit = yaml.safe_load((unit_dir / "unit.yaml").read_text(encoding="utf-8"))
    unit["lessons"].append("05-hidden.yaml")
